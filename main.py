...
settings = json.load(open("config/settings.json", 'r', encoding='utf8')) #讀取你的setting.json
#------------------------------------------------------------------
intents = discord.Intents.default()
# 啟用需要的 Intents 設定成 True
intents.members = True           # 成員加入/退出記錄
intents.message_content = True   # 訊息內容分析（如觸發自動回應）
...
class ManagementCommand(commands.Cog):
    ...
    #-----------------------COG載入重載卸載控制區-------------------------
    @discord.app_commands.command(name="載入模組", description="載入`extension`模組")
    ...
    async def load(self, interaction: discord.Interaction, extension: str):
        if interaction.guild is None:
            await interaction.response.send_message(embed=discord.Embed(title="權限拒絕", description="不可在非伺服器中使用此指令", color=0xff0000), ephemeral=False)
            return
        if not interaction.user.guild_permissions.administrator:
            embed = discord.Embed(title="權限不足", description="你沒有足夠的權限使用這個命令", color=0xff0000)
    ...
    @discord.app_commands.command(name="卸載模組", description="卸載`extension`模組")
    ...
    async def unload(self, interaction: discord.Interaction, extension: str):
        ...
        try:
            ...
            logger.warning(f"尚未載入：{extension}")
        except Exception as e:
            await interaction.response.send_message(embed=discord.Embed(title="錯誤：卸載模組時發生未知錯誤", description=f"錯誤訊息:\n{e}", color=0xff0000), ephemeral=True)
            logger.error(f"未知錯誤：{e}\n{traceback.format_exc()}")

    @discord.app_commands.command(name="重新載入模組", description="重新載入`extension`模組")
